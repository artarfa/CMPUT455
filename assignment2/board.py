...
class GoBoard(object):
    ...
    def end_of_game(self) -> bool:
        if self.get_empty_points().size == 0 or self.detect_five_in_a_row() != EMPTY or self.black_captures >= 10 or self.white_captures >= 10:
            return True
        return False
        ...
        for offset in offsets:
            if self.board[point+offset] == O and self.board[point+(offset*2)] == O and self.board[point+(offset*3)] == color:
                ...
                if color == BLACK:
                    ...
                    self.white_captures += 2
                self.last_capture = O
                self.capture = (point+offset, point+(offset*2))
        return True
    
    def check_capture(self, point: GO_POINT, color: GO_COLOR) -> int:
        cap = 0
        O = opponent(color)
        offsets = [1, -1, self.NS, -self.NS, self.NS+1, -(self.NS+1), self.NS-1, -self.NS+1]
        for offset in offsets:
    ...
    def check_win(self, point: GO_POINT, color: GO_COLOR) -> int:
        win = 0
        pconnect: List[GO_POINT] = self.neighbors_of_color(point, color)
        win = len(pconnect)
        temp = 0
        for piece in pconnect:
            vector = piece - point
            connect = 2
            for i in range(1, 4):
                piece = self.get_color(piece + vector * i)
                if piece == color:
                    connect += 1
                elif piece == opponent(color) or piece == BORDER:
                    break
            for i in range(2, 4):
                piece = self.get_color(piece - vector * i)
                if piece == color:
    ...
    def undo(self ,point):
        ...
        if self.last_capture != EMPTY:
            self.board[self.capture[0]] = self.last_capture
            self.board[self.capture[1]] = self.last_capture
            if self.last_capture == WHITE:
                self.black_captures -= 2
            else:
                self.white_captures -= 2
    ...
    def best_moves(self):
        ...
        return sorted(arr, key=self.eval_score, reverse=True)

    def eval_score(self, move):
    ...
    def eval(self):
        score = 0
        O = opponent(self.current_player)
        ...
        for line in lines:
            for i in range(len(line) - 5):
                ...
                if player < 1 or oplayer < 1:
                    score += 10 ** player - 10 ** oplayer
        if score != 0:
            if self.current_player == WHITE:
                score += (self.white_captures - self.black_captures) * 5
            else:
                score += (self.black_captures - self.white_captures) * 5
        return score
        # score = 0

        # # Capture check
        # score += 20 * self.check_capture(point, color)
        
        # # Win check
        # score += 100 * self.check_win(point, color)

        # # Block win
        # score += 50 * self.check_win(point, opponent(color))

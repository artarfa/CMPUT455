...
class GtpConnection:
    def __init__(self, go_engine: GoEngine, board: GoBoard, debug_mode: bool = False) -> None:
        ...
        self.timelimit = 1
        self.table = {}
        self.hasher = self.inithash(self.board.size)
    ...
    def boardsize_cmd(self, args: List[str]) -> None:
        ...
        if boardsize != self.board.size:
            self.hasher = self.inithash(boardsize)
            self.table = {}
    ...
    def genmove_cmd(self, args: List[str]) -> None:
        ...
        result, move = self.solve_cmd("genmove")
        if result == "unknown" or (result != color and result not in ("unknown", "draw")):
    ...
    def solve_cmd(self, args: List[str]):
        """ Implement this function for Assignment 2 """
        result, move = self.solver()
        if args == "genmove":
            return result, move
        if move == None:
            self.respond(result)
        else:
            move = format_point(point_to_coord(move, self.board.size)).lower()
            self.respond("%s %s" % (result, move))
    ...
    def solver(self):
        ...
        try:
            ...
            result, move = self.alphabeta(rootState, -100000000, 100000000, depth)
            if result == 0:
                 return "draw", move
            elif self.board.current_player == BLACK and result > 0:
                return "b", move
            elif self.board.current_player == WHITE and result > 0:
                return "w", move
            elif self.board.current_player == BLACK and result < 0:
                return "w", None
            elif self.board.current_player == WHITE and result < 0:
    ...
    def alphabeta(self, state, alpha, beta, depth):
        code = self.zhash(GoBoardUtil.get_twoD_board(state).flatten())
        ...
        if state.end_of_game() or depth == 0:
            result = (state.end_eval(), None)
            self.table[code] = result
            return result
        
        moves = state.best_moves()
        result = moves[0]

        for m in moves:
            state.play_move(m, state.current_player)
            (value, _) = self.alphabeta(state, -beta, -alpha, depth - 1)
            value = -value
            if value > alpha:
                alpha = value
            state.undo(m)
    ...
    def inithash(self, size):
        self.index = size * size
        self.array = [[random.getrandbits(64) for j in range(3)] for i in range(self.index)]

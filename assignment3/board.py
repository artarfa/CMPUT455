...
class GoBoard(object):
    ...
    def get_captures(self, color: GO_COLOR):
        if color == BLACK:
    ...
    def neighbors_of_color(self, point: GO_POINT, color: GO_COLOR) -> List:
        ...
        nbc: List[GO_POINT] = []
        for nb in self._allneighbors(point):
    ...
    def simulation(self): 
        if not self.end_of_game():
            ...
            while not self.end_of_game():
                ...
                for i in allMoves:
                    for _ in range(10):
                        self.play_move(i, self.current_player)


                    if self.get_captures(BLACK) >= 10 or self.detect_five_in_a_row() == BLACK:
                        return BLACK
                    elif self.get_captures(WHITE) >= 10 or self.detect_five_in_a_row() == WHITE:
                        return WHITE
                    return EMPTY
    ...
    def get_potential_moves(self):
        ...
        moves: List[GO_POINT] = []
        for move in points:
            for point in self._allneighbors(move):
                if self.get_color(point) in (BLACK, WHITE):
                    moves.append(move)
                    break
        return moves
    ...
    def analyze(self, move):
        ...
        for point in self.neighbors_of_color(move, color):
            connect = 2
            slot = 0
            vector = point - move
            for i in range(1, 4):
                test = self.get_color(point + vector * i)
                if test == color:
                    connect += 1
                else:
                    if test == EMPTY:
            ...
            if connect >= 5:
                # win by connect 5
                return 1
            elif connect == 4 and slot == 2 and pattern != 2:
                # open 4, and both sides are open
                pattern = 3
            # print(move, point, vector, self.board[point+vector], self.board[point + vector * 2])
            if self.board[point+vector] == color \
                and self.board[point+(vector*2)] == O \
                and self.get_captures(O) >= 8:
                    # block by anti-capture
                    pattern = 2

        for point in self.neighbors_of_color(move, O):
            vector = point - move
            if self.board[point+vector] == O \
                and self.board[point+(vector*2)] == color:
                    if self.get_captures(color) >= 8:
                        # win by capture
                        return 1
                    if pattern not in (2, 3):
                        # capturable
                        pattern = 4
            else:
                connect = 2
                vector = point - move
                for i in range(1, 4):
                    if self.get_color(point + vector * i) == O:
                        connect += 1
                    else:
                        break
                for i in range(2, 4):

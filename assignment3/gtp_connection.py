...
class GtpConnection:
    ...
    def policytype_cmd(self, args: List[str]):
        policies = ['random', 'rule_based']
    
        if args[0] in policies:
            self.policytype = args[0]
            self.respond()
        else:
            self.respond("policy: random or rule_based")
    
    def sort_move(self, moves):
        gtp_moves = [format_point(point_to_coord(move, self.board.size)).lower() for move in moves]
        return " ".join(sorted(gtp_moves))

    def policy_moves_cmd(self, args: List[str]):
        legal_moves = self.board.get_empty_points()
        sorted_moves = self.sort_move(legal_moves)
        ...
        if self.policytype == "random":
            ...
            # print(moves[0])
            # gtp_moves = [format_point(point_to_coord(move, self.board.size)).lower() for move in moves[0]]
            # print(gtp_moves)

            if len(moves[1]) != 0:
                self.respond("Win " + self.sort_move(moves[1]))
            elif len(moves[2]) != 0:
                moves[2] += moves[4]    # because capture can also block win
                self.respond("BlockWin " + self.sort_move(moves[2]))
            elif len(moves[3]) != 0:
                self.respond("OpenFour " + self.sort_move(moves[3]))
            elif len(moves[4]) != 0:
                self.respond("Capture " + self.sort_move(moves[4]))        

            else:
                self.respond("Random " + sorted_moves)

        else:
            self.respond("there is a bug switching policytype.")     
    ...
    def genmove_cmd(self, args: List[str]) -> None:
        ...
        if self.policytype == 'random':
            ...
            x = format_point(point_to_coord(move, self.board.size))
            self.play_cmd([board_color, x, 'print_move'])
            return
        ...
        if self.policytype == "rule_based":
            ...
            for move in moves[0]:
                ...
                if result != 0:
                    moves[result].append(move)
            moveset = moves[1] + moves[2] + moves[3] + moves[4]
            if len(moveset) > 0:
                move = moveset[0]

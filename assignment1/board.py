...
class GoBoard(object):
    ...
    def reset(self, size: int) -> None:
        ...
        self.NS: int = size + 1
        self.WE: int = 1
        self.ko_recapture: GO_POINT = None
        self.last_move: GO_POINT = None
        self.last2_move: GO_POINT = None
    ...
    def win(self, point, color):
        def check_direction(neighbors, diff_condition):
            counter = 1
            for nb in neighbors:
                diff = nb - point
                counter = 1
                if abs(diff) == diff_condition:
        ...
        if (
            check_direction(neighbors, 1)
            or check_direction(neighbors, self.NS)
            or check_direction(diag_neighbors, self.NS + 1)
            or check_direction(diag_neighbors, self.NS - 1)
        ):
            return True
    ...
    def result(self):
        if len(self.get_empty_points()) == self.size*self.size:
            return "unkown"
        color = opponent(self.current_player)
        if self.win(self.last_move, color):
            if color == BLACK:
                return "black"
            if color == WHITE:
                return "white"
